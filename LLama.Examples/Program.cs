...
var modelPath = "D:\\development\\llama\\weights\\wizard-vicuna-13B.ggmlv3.q4_1.bin";
var prompt = File.ReadAllText("Assets/chat-with-bob.txt").Trim();
//string prompt = " Qeustion: how to do binary search for an array in C#? Answer: ";

InteractiveExecutor ex = new(new LLamaModel(new ModelParams(modelPath, 1024, seed: 1337, gpuLayerCount: 5)));

var session = new ChatSession(ex).WithOutputTransform(new KeywordTextOutputStreamTransform(new[] { "User:", "Bob:" }));
...
while (prompt != "skip")
{
    ...
    if (prompt == "save")
    {
        ...
        ex.Model.Dispose();
        ex = new InteractiveExecutor(new LLamaModel(new ModelParams(modelPath, 1024, seed: 1337, gpuLayerCount: 5)));
        session = new ChatSession(ex).WithOutputTransform(
            new KeywordTextOutputStreamTransform(new[] { "User:", "Bob:" }));
