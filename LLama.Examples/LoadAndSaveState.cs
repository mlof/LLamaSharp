...
namespace LLama.Examples
{
    public class SaveAndLoadState : IDisposable
    {
        ...
        {
            _executor = new InteractiveExecutor(new LLamaModel(new ModelParams(modelPath)));
            foreach (var text in _executor.Infer(prompt,
        ...
        {
            var sessionParams = new InferenceParams { Temperature = 0.2f, AntiPrompts = new List<string> { "user:" } };
