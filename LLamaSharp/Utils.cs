...
namespace LLama
{
    ...
    internal static class Utils
    {
        public static SafeLLamaContextHandle InitLLamaContextFromModelParams(ModelParams @params)
        {
            ...
            {
                var err = NativeApi.llama_apply_lora_from_file(ctx, @params.LoraAdapter,
        ...
        public static IEnumerable<llama_token> Tokenize(SafeLLamaContextHandle ctx, string text, bool add_bos,
            Encoding encoding)
        {
            var cnt = encoding.GetByteCount(text);
            var res = new llama_token[cnt + (add_bos ? 1 : 0)];
            var n = NativeApi.llama_tokenize(ctx, text, encoding, res, res.Length, add_bos);
            if (n < 0)
            {
                throw new RuntimeError(
                    "Error happened during tokenization. It's possibly caused by wrong encoding. Please try to " +
        ...
        public static unsafe int Eval(SafeLLamaContextHandle ctx, llama_token[] tokens, int startIndex, int n_tokens,
